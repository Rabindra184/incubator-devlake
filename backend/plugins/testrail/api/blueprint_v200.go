...
func makePipelinePlanV200(
	subtaskMetas []plugin.SubTaskMeta,
	scopeDetails []*srvhelper.ScopeDetail[models.TestrailProject, models.TestrailScopeConfig],
	...
	for i, scopeDetail := range scopeDetails {
		...
		// construct task options for testrail
		task, err := api.MakePipelinePlanTask(
			"testrail",
			subtaskMetas,
			scopeConfig.Entities,
			tasks.TestrailOptions{
...
func makeScopesV200(
	...
	for _, scopeDetail := range scopeDetails {
		...
		// Return the TestrailProject itself as it implements plugin.Scope
