...
var ConvertProjectsMeta = plugin.SubTaskMeta{
	...
	Description:      "Convert tool layer table testrail_projects into domain layer table qa_projects",
...
func ConvertProjects(taskCtx plugin.SubTaskContext) errors.Error {
	...
	cursor, err := db.Cursor(
		dal.From(&models.TestrailProject{}),
		dal.Where("connection_id = ? AND id = ?", data.Options.ConnectionId, projectId),
	)
	if err != nil {
		return err
	}
	...
	converter, err := helper.NewDataConverter(helper.DataConverterArgs{
		InputRowType: reflect.TypeOf(models.TestrailProject{}),
		Input:        cursor,
		...
		Convert: func(inputRow interface{}) ([]interface{}, errors.Error) {
			project := inputRow.(*models.TestrailProject)
			domainProject := &qa.QaProject{
				DomainEntityExtended: domainlayer.DomainEntityExtended{
					Id: projectIdGen.Generate(project.ConnectionId, project.Id),
				},
				Name: project.Name,
			}
