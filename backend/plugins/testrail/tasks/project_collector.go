...
var CollectProjectsMeta = plugin.SubTaskMeta{
	...
	Description:      "Collect projects data from Testrail api",
...
func CollectProjects(taskCtx plugin.SubTaskContext) errors.Error {
	...
	collector, err := helper.NewApiCollector(helper.ApiCollectorArgs{
		...
		PageSize:    100,
		Incremental: false,
		UrlTemplate: "index.php?/api/v2/get_project/{{ .Params.ProjectId }}",
		GetTotalPages: func(res *http.Response, args *helper.ApiCollectorArgs) (int, errors.Error) {
			return 1, nil
		},
		ResponseParser: func(res *http.Response) ([]json.RawMessage, errors.Error) {
			var rawMsg json.RawMessage
			err := helper.UnmarshalResponse(res, &rawMsg)
			if err != nil {
				return nil, err
			}
			return []json.RawMessage{rawMsg}, nil
		},
	})
